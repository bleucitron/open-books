...
var require_path = __commonJS({
  "node_modules/doc-path/dist/path.js"(exports, module2) {
    ...
    function evaluatePath(t, e) {
      ...
      return r >= 0 && !t[e] ? Array.isArray(t[a]) ? t[a].map((t2) => evaluatePath(t2, n)) : evaluatePath(t[a], n) : Array.isArray(t) ? t.map((t2) => evaluatePath(t2, e)) : r >= 0 && e !== a && t[a] ? evaluatePath(t[a], n) : r === -1 && t[a] && !t[e] ? t[a] : t[e];
    }
  ...
  "node_modules/deeks/lib/deeks.js"(exports, module2) {
    ...
    function deepKeysFromList(list, options2) {
      ...
      return list.map((document) => {
        if (utils.isObject(document)) {
          return deepKeys(document, options2);
        }
        return [];
      });
    }
    function generateDeepKeysList(heading, data, options2) {
      let keys2 = Object.keys(data).map((currentKey) => {
        let keyName = buildKeyName(heading, escapeNestedDotsIfSpecified(currentKey, options2));
        if (isDocumentToRecurOn(data[currentKey])) {
          return generateDeepKeysList(keyName, data[currentKey], options2);
        } else if (options2.expandArrayObjects && isArrayToRecurOn(data[currentKey])) {
          return processArrayKeys(data[currentKey], keyName, options2);
        } else if (options2.ignoreEmptyArrays && isArrayToRecurOn(data[currentKey]) && !data[currentKey].length) {
          return [];
        }
        return keyName;
      });
...
var require_utils2 = __commonJS({
  "node_modules/json-2-csv/lib/utils.js"(exports, module2) {
    ...
    function convert(params) {
      let { options: options2, callback } = parseArguments(params.callback, params.options);
      options2 = buildOptions(options2);
      let converter = new params.converter(options2), valid = validateParameters({
        data: params.data,
        callback,
...
var require_json2csv = __commonJS({
  "node_modules/json-2-csv/lib/json2csv.js"(exports, module2) {
    ...
    var Json2Csv = function(options2) {
      const wrapDelimiterCheckRegex = new RegExp(options2.delimiter.wrap, "g"), crlfSearchRegex = /\r?\n|\r/, valueParserFn = options2.parseValue && typeof options2.parseValue === "function" ? options2.parseValue : recordFieldValueToString, expandingWithoutUnwinding = options2.expandArrayObjects && !options2.unwindArrays, deeksOptions = {
      ...
      function filterExcludedKeys(keyPaths) {
        if (options2.excludeKeys) {
          return keyPaths.filter((keyPath) => !options2.excludeKeys.includes(keyPath));
        }
        return keyPaths;
      }
      ...
      function generateCsvHeader(params) {
        ...
        params.header = params.headerFields.map(function(field) {
          const headerKey = fieldTitleMapKeys.includes(field) ? options2.fieldTitleMap[field] : field;
      ...
      function retrieveHeaderFields(data) {
        ...
        if (options2.keys && !options2.unwindArrays) {
          return Promise.resolve(options2.keys).then(filterExcludedKeys).then(sortHeaderFields);
        }
      ...
      function retrieveRecordFieldData(record, fields2) {
        ...
        fields2.forEach((field) => {
          ...
          if (!utils.isUndefined(options2.emptyFieldValue) && utils.isEmptyField(recordFieldValue)) {
      ...
      function generateCsvFromComponents(params) {
        let header = params.header, records = params.records, csv = (options2.excelBOM ? constants.values.excelBOM : "") + (options2.prependHeader ? header + options2.delimiter.eol : "") + records;
...
var require_csv2json = __commonJS({
  "node_modules/json-2-csv/lib/csv2json.js"(exports, module2) {
    ...
    var Csv2Json = function(options2) {
      const escapedWrapDelimiterRegex = new RegExp(options2.delimiter.wrap + options2.delimiter.wrap, "g"), excelBOMRegex = new RegExp("^" + constants.values.excelBOM), valueParserFn = options2.parseValue && typeof options2.parseValue === "function" ? options2.parseValue : JSON.parse;
      ...
      function retrieveHeading(lines) {
        let params = { lines }, headerRow = params.lines[0];
        params.headerFields = headerRow.map((headerKey, index2) => ({
          value: processHeaderKey(headerKey),
          index: index2
        }));
        if (options2.keys) {
          params.headerFields = params.headerFields.filter((headerKey) => options2.keys.includes(headerKey.value));
        }
        return params;
      }
      ...
      function splitLines(csv) {
        let lines = [], splitLine = [], character, charBefore, charAfter, nextNChar, lastCharacterIndex = csv.length - 1, eolDelimiterLength = options2.delimiter.eol.length, stateVariables = {
        ...
        while (index2 < csv.length) {
          ...
          if ((nextNChar === options2.delimiter.eol && !stateVariables.insideWrapDelimiter || index2 === lastCharacterIndex) && charBefore === options2.delimiter.field) {
            if (nextNChar === options2.delimiter.eol && stateVariables.startIndex === index2) {
              splitLine.push("");
          ...
          } else if (index2 === lastCharacterIndex && character === options2.delimiter.field) {
            let parsedValue = csv.substring(stateVariables.startIndex, index2);
            splitLine.push(parsedValue);
            splitLine.push("");
            lines.push(splitLine);
          } else if (index2 === lastCharacterIndex || nextNChar === options2.delimiter.eol && (!stateVariables.insideWrapDelimiter || stateVariables.insideWrapDelimiter && charBefore === options2.delimiter.wrap && !stateVariables.justParsedDoubleQuote)) {
            let toIndex = index2 !== lastCharacterIndex || charBefore === options2.delimiter.wrap ? index2 : void 0;
            ...
            stateVariables.parsingValue = true;
            stateVariables.insideWrapDelimiter = charAfter === options2.delimiter.wrap;
          } else if ((charBefore !== options2.delimiter.wrap || stateVariables.justParsedDoubleQuote && charBefore === options2.delimiter.wrap) && character === options2.delimiter.wrap && utils.getNCharacters(csv, index2 + 1, eolDelimiterLength) === options2.delimiter.eol) {
            stateVariables.insideWrapDelimiter = false;
            stateVariables.parsingValue = false;
          } else if (character === options2.delimiter.wrap && (index2 === 0 || utils.getNCharacters(csv, index2 - eolDelimiterLength, eolDelimiterLength) === options2.delimiter.eol)) {
            stateVariables.insideWrapDelimiter = true;
            stateVariables.parsingValue = true;
            stateVariables.startIndex = index2;
          } else if (character === options2.delimiter.wrap && charAfter === options2.delimiter.field) {
            splitLine.push(csv.substring(stateVariables.startIndex, index2 + 1));
            ...
            stateVariables.parsingValue = true;
            stateVariables.startIndex = index2;
          } else if (character === options2.delimiter.wrap && charAfter === options2.delimiter.wrap) {
            index2 += 2;
      ...
      function createDocument(keys2, line) {
        return keys2.reduce((document, key) => {
      ...
      function parseValue(value) {
        try {
          if (utils.isStringRepresentation(value, options2) && !utils.isDateRepresentation(value)) {
            return value;
          }
...
var require_converter = __commonJS({
  "node_modules/json-2-csv/lib/converter.js"(exports, module2) {
    ...
    var utils = require_utils2();
    module2.exports = {
      json2csv: (data, callback, options2) => convert(Json2Csv, data, callback, options2),
      csv2json: (data, callback, options2) => convert(Csv2Json, data, callback, options2),
      ...
      json2csvPromisified: (json, options2) => deprecatedAsync(Json2Csv, "json2csvPromisified()", "json2csvAsync()", json, options2),
      csv2jsonPromisified: (csv, options2) => deprecatedAsync(Csv2Json, "csv2jsonPromisified()", "csv2jsonAsync()", csv, options2)
    };
    ...
    function convertAsync(converter, data, options2) {
      return new Promise((resolve2, reject) => convert(converter, data, (err, data2) => {
        if (err) {
          return reject(err);
        }
    ...
    function deprecatedAsync(converter, deprecatedName, replacementName, data, options2) {
      ...
      html(src2) {
        ...
        if (cap) {
          ...
          if (this.options.sanitize) {
            ...
            token2.text = this.options.sanitizer ? this.options.sanitizer(cap[0]) : escape2(cap[0]);
            token2.tokens = [];
            this.lexer.inline(token2.text, token2.tokens);
          }
          return token2;
    ...
    module2.exports = class Lexer {
      ...
      inlineTokens(src2, tokens = []) {
        ...
        while (src2) {
          ...
          if (!this.state.inLink && (token2 = this.tokenizer.url(src2, mangle))) {
            src2 = src2.substring(token2.raw.length);
            tokens.push(token2);
            continue;
          }
  ...
  "node_modules/marked/src/Parser.js"(exports, module2) {
    ...
    module2.exports = class Parser {
      ...
      parse(tokens, top = true) {
        let out = "", i, j, k, l2, l3, row, cell, header, body, token2, ordered, start2, loose, itemBody, item, checked, task, checkbox, ret;
        ...
        for (i = 0; i < l; i++) {
          ...
          switch (token2.type) {
            ...
            case "table": {
              ...
              for (j = 0; j < l2; j++) {
                cell += this.renderer.tablecell(this.parseInline(token2.header[j].tokens), { header: true, align: token2.align[j] });
              }
              ...
              for (j = 0; j < l2; j++) {
                ...
                l3 = row.length;
                for (k = 0; k < l3; k++) {
                  cell += this.renderer.tablecell(this.parseInline(row[k].tokens), { header: false, align: token2.align[k] });
                }
...
var template = ({ head, body }) => '<!DOCTYPE html>\n<html lang="en">\n  <head>\n    <meta charset="utf-8" />\n    <link rel="icon" href="/favicon.png" />\n    <meta name="viewport" content="width=device-width, initial-scale=1" />\n    ' + head + `
    ...
    <script
      src="https://kit.fontawesome.com/07a38673bd.js"
...
function init(settings = default_settings) {
  ...
  options$1 = {
    ...
    dev: false,
    entry: {
      file: assets + "/_app/start-ae37f2bd.js",
      css: [assets + "/_app/assets/start-c446e5f0.css", assets + "/_app/assets/vendor-1ced044c.css"],
      js: [assets + "/_app/start-ae37f2bd.js", assets + "/_app/chunks/vendor-f155ec50.js", assets + "/_app/chunks/singletons-12a22614.js"]
    },
    fetched: void 0,
    floc: false,
...
var empty = () => ({});
var manifest = {
  assets: [{ "file": "favicon.png", "size": 1571, "type": "image/png" }, { "file": "global.css", "size": 628, "type": "text/css" }, { "file": "manifest.json", "size": 153, "type": "application/json" }, { "file": "style.css", "size": 401, "type": "text/css" }],
...
var module_lookup = {
  ...
  "src/routes/budgets/index.svelte": () => Promise.resolve().then(function() {
    return index;
  }),
...
var metadata_lookup = { "src/routes/__layout.svelte": { "entry": "pages/__layout.svelte-fe98e9e5.js", "css": ["assets/pages/__layout.svelte-d5e432b4.css", "assets/vendor-1ced044c.css"], "js": ["pages/__layout.svelte-fe98e9e5.js", "chunks/vendor-f155ec50.js"], "styles": [] }, "src/routes/__error.svelte": { "entry": "pages/__error.svelte-2fdacd79.js", "css": ["assets/pages/__error.svelte-51aeb449.css", "assets/vendor-1ced044c.css"], "js": ["pages/__error.svelte-2fdacd79.js", "chunks/vendor-f155ec50.js"], "styles": [] }, "src/routes/index.svelte": { "entry": "pages/index.svelte-69e2bad4.js", "css": ["assets/vendor-1ced044c.css", "assets/pages/home/index.svelte-f5eba495.css", "assets/Spinner-10ebec9c.css"], "js": ["pages/index.svelte-69e2bad4.js", "chunks/vendor-f155ec50.js", "pages/home/index.svelte-1b222b6f.js", "chunks/Spinner-d90d95ed.js"], "styles": [] }, "src/routes/budgets/index.svelte": { "entry": "pages/budgets/index.svelte-5df6de23.js", "css": ["assets/pages/budgets/index.svelte-8fb6811f.css", "assets/vendor-1ced044c.css", "assets/Spinner-10ebec9c.css"], "js": ["pages/budgets/index.svelte-5df6de23.js", "chunks/vendor-f155ec50.js", "chunks/Spinner-d90d95ed.js", "chunks/singletons-12a22614.js"], "styles": [] }, "src/routes/about.svelte": { "entry": "pages/about.svelte-81bf6b91.js", "css": ["assets/pages/about.svelte-50a1a333.css", "assets/vendor-1ced044c.css"], "js": ["pages/about.svelte-81bf6b91.js", "chunks/vendor-f155ec50.js"], "styles": [] }, "src/routes/home/index.svelte": { "entry": "pages/home/index.svelte-1b222b6f.js", "css": ["assets/pages/home/index.svelte-f5eba495.css", "assets/vendor-1ced044c.css", "assets/Spinner-10ebec9c.css"], "js": ["pages/home/index.svelte-1b222b6f.js", "chunks/vendor-f155ec50.js", "chunks/Spinner-d90d95ed.js"], "styles": [] } };
...
var css$i = {
  code: "header.svelte-1erhb9q.svelte-1erhb9q.svelte-1erhb9q{display:flex;justify-content:space-between;height:3rem}a.svelte-1erhb9q.svelte-1erhb9q.svelte-1erhb9q{display:flex;align-items:center;margin:0 0.5rem;cursor:pointer}.home.svelte-1erhb9q.svelte-1erhb9q.svelte-1erhb9q{font-size:1.5rem}.home.svelte-1erhb9q>div.svelte-1erhb9q.svelte-1erhb9q{display:flex;align-items:baseline}.home.svelte-1erhb9q h1.svelte-1erhb9q.svelte-1erhb9q{color:black;margin:0 0.5rem;font-size:1.4rem}.home.svelte-1erhb9q h2.svelte-1erhb9q.svelte-1erhb9q{color:#bbb;font-size:1rem;display:flex;align-items:center}a.svelte-1erhb9q.svelte-1erhb9q.svelte-1erhb9q{transition:color 0.3s ease-in-out}a.svelte-1erhb9q.svelte-1erhb9q.svelte-1erhb9q:hover{color:coral}a.current.svelte-1erhb9q.svelte-1erhb9q.svelte-1erhb9q{color:cornflowerblue;cursor:unset}a.current.svelte-1erhb9q.svelte-1erhb9q.svelte-1erhb9q:hover{color:cornflowerblue;opacity:unset}nav.svelte-1erhb9q.svelte-1erhb9q.svelte-1erhb9q{display:flex;justify-content:flex-end;align-items:stretch;padding:0.5rem;color:black;background:white;z-index:1}nav.svelte-1erhb9q a.svelte-1erhb9q.svelte-1erhb9q{transition:color 0.3s ease-in-out}nav.svelte-1erhb9q a.svelte-1erhb9q.svelte-1erhb9q:hover{color:coral}nav.svelte-1erhb9q a.current.svelte-1erhb9q.svelte-1erhb9q{color:cornflowerblue;cursor:unset}nav.svelte-1erhb9q a.current.svelte-1erhb9q.svelte-1erhb9q:hover{color:cornflowerblue;opacity:unset}nav.svelte-1erhb9q a.svelte-1erhb9q i.svelte-1erhb9q{font-size:1.2rem;display:flex;justify-content:center;align-items:center;border-radius:50%}",
  map: `{"version":3,"file":"Nav.svelte","sources":["Nav.svelte"],"sourcesContent":["<script lang=\\"ts\\">export let path;\\n<\/script>\\n\\n<header>\\n  <a href=\\"/\\" class=\\"home\\" class:current={path === 'home'}>\\n    <div>\\n      <i class=\\"fas fa-book-open\\" />\\n      <h1>Livres ouverts</h1>\\n      <h2>Les donn\xE9es budg\xE9taires des communes</h2>\\n    </div>\\n  </a>\\n  <nav class=\\"Nav\\">\\n    <a href=\\"/about\\" class:current={path === 'about'}> \xC0 propos </a>\\n    <a href=\\"https://github.com/iOiurson/open-books\\">\\n      <i class=\\"fab fa-github\\" />\\n    </a>\\n  </nav>\\n</header>\\n\\n<style lang=\\"scss\\">header {\\n  display: flex;\\n  justify-content: space-between;\\n  height: 3rem;\\n}\\n\\na {\\n  display: flex;\\n  align-items: center;\\n  margin: 0 0.5rem;\\n  cursor: pointer;\\n}\\n\\n.home {\\n  font-size: 1.5rem;\\n}\\n.home > div {\\n  display: flex;\\n  align-items: baseline;\\n}\\n.home h1 {\\n  color: black;\\n  margin: 0 0.5rem;\\n  font-size: 1.4rem;\\n}\\n.home h2 {\\n  color: #bbb;\\n  font-size: 1rem;\\n  display: flex;\\n  align-items: center;\\n}\\n\\na {\\n  transition: color 0.3s ease-in-out;\\n}\\na:hover {\\n  color: coral;\\n}\\na.current {\\n  color: cornflowerblue;\\n  cursor: unset;\\n}\\na.current:hover {\\n  color: cornflowerblue;\\n  opacity: unset;\\n}\\n\\nnav {\\n  display: flex;\\n  justify-content: flex-end;\\n  align-items: stretch;\\n  padding: 0.5rem;\\n  color: black;\\n  background: white;\\n  z-index: 1;\\n}\\nnav a {\\n  transition: color 0.3s ease-in-out;\\n}\\nnav a:hover {\\n  color: coral;\\n}\\nnav a.current {\\n  color: cornflowerblue;\\n  cursor: unset;\\n}\\nnav a.current:hover {\\n  color: cornflowerblue;\\n  opacity: unset;\\n}\\nnav a i {\\n  font-size: 1.2rem;\\n  display: flex;\\n  justify-content: center;\\n  align-items: center;\\n  border-radius: 50%;\\n}</style>\\n"],"names":[],"mappings":"AAmBmB,MAAM,6CAAC,CAAC,AACzB,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,aAAa,CAC9B,MAAM,CAAE,IAAI,AACd,CAAC,AAED,CAAC,6CAAC,CAAC,AACD,OAAO,CAAE,IAAI,CACb,WAAW,CAAE,MAAM,CACnB,MAAM,CAAE,CAAC,CAAC,MAAM,CAChB,MAAM,CAAE,OAAO,AACjB,CAAC,AAED,KAAK,6CAAC,CAAC,AACL,SAAS,CAAE,MAAM,AACnB,CAAC,AACD,oBAAK,CAAG,GAAG,8BAAC,CAAC,AACX,OAAO,CAAE,IAAI,CACb,WAAW,CAAE,QAAQ,AACvB,CAAC,AACD,oBAAK,CAAC,EAAE,8BAAC,CAAC,AACR,KAAK,CAAE,KAAK,CACZ,MAAM,CAAE,CAAC,CAAC,MAAM,CAChB,SAAS,CAAE,MAAM,AACnB,CAAC,AACD,oBAAK,CAAC,EAAE,8BAAC,CAAC,AACR,KAAK,CAAE,IAAI,CACX,SAAS,CAAE,IAAI,CACf,OAAO,CAAE,IAAI,CACb,WAAW,CAAE,MAAM,AACrB,CAAC,AAED,CAAC,6CAAC,CAAC,AACD,UAAU,CAAE,KAAK,CAAC,IAAI,CAAC,WAAW,AACpC,CAAC,AACD,8CAAC,MAAM,AAAC,CAAC,AACP,KAAK,CAAE,KAAK,AACd,CAAC,AACD,CAAC,QAAQ,6CAAC,CAAC,AACT,KAAK,CAAE,cAAc,CACrB,MAAM,CAAE,KAAK,AACf,CAAC,AACD,CAAC,qDAAQ,MAAM,AAAC,CAAC,AACf,KAAK,CAAE,cAAc,CACrB,OAAO,CAAE,KAAK,AAChB,CAAC,AAED,GAAG,6CAAC,CAAC,AACH,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,QAAQ,CACzB,WAAW,CAAE,OAAO,CACpB,OAAO,CAAE,MAAM,CACf,KAAK,CAAE,KAAK,CACZ,UAAU,CAAE,KAAK,CACjB,OAAO,CAAE,CAAC,AACZ,CAAC,AACD,kBAAG,CAAC,CAAC,8BAAC,CAAC,AACL,UAAU,CAAE,KAAK,CAAC,IAAI,CAAC,WAAW,AACpC,CAAC,AACD,kBAAG,CAAC,+BAAC,MAAM,AAAC,CAAC,AACX,KAAK,CAAE,KAAK,AACd,CAAC,AACD,kBAAG,CAAC,CAAC,QAAQ,8BAAC,CAAC,AACb,KAAK,CAAE,cAAc,CACrB,MAAM,CAAE,KAAK,AACf,CAAC,AACD,kBAAG,CAAC,CAAC,sCAAQ,MAAM,AAAC,CAAC,AACnB,KAAK,CAAE,cAAc,CACrB,OAAO,CAAE,KAAK,AAChB,CAAC,AACD,kBAAG,CAAC,gBAAC,CAAC,CAAC,eAAC,CAAC,AACP,SAAS,CAAE,MAAM,CACjB,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,MAAM,CACvB,WAAW,CAAE,MAAM,CACnB,aAAa,CAAE,GAAG,AACpB,CAAC"}`
};
var Nav = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<header class="${"svelte-1erhb9q"}"><a href="${"/"}" class="${["home svelte-1erhb9q", path === "home" ? "current" : ""].join(" ").trim()}"><div class="${"svelte-1erhb9q"}"><i class="${"fas fa-book-open svelte-1erhb9q"}"></i>
      <h1 class="${"svelte-1erhb9q"}">Livres ouverts</h1>
      <h2 class="${"svelte-1erhb9q"}">Les donn\xE9es budg\xE9taires des communes</h2></div></a>
  <nav class="${"Nav svelte-1erhb9q"}"><a href="${"/about"}" class="${["svelte-1erhb9q", path === "about" ? "current" : ""].join(" ").trim()}">\xC0 propos </a>
    <a href="${"https://github.com/iOiurson/open-books"}" class="${"svelte-1erhb9q"}"><i class="${"fab fa-github svelte-1erhb9q"}"></i></a></nav>
</header>`;
});
var GoogleAnalytics = create_ssr_component(($$result, $$props, $$bindings, slots) => {
...
var _layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  segment = $page.path.split("/")[1];
  $$unsubscribe_page();
  return `${validate_component(GoogleAnalytics, "GoogleAnalytics").$$render($$result, {}, {}, {})}
<div class="${"page svelte-1hseanh"}">${segment !== "budgets" ? `${validate_component(Nav, "Nav").$$render($$result, { path: segment || "home" }, {}, {})}` : ``}
...
<h1 class="${"svelte-q8ws67"}">${escape(status)}</h1>
...
function makeGetCitiesEndpoint(text) {
...
function aggregateData(records, tree2) {
  const aggregate = Object.values(tree2).map((fonction2) => {
    let { code: code2, subTree } = fonction2;
    let obnetdeb;
    let obnetcre;
    let oobdeb;
    let oobcre;
    ...
    if (!subTree) {
      ...
      const values = Object.values(subAgg);
...
function buildParamString$1(paramByKey) {
  return Object.entries(paramByKey).map(([key, value]) => {
    if (Array.isArray(value)) {
...
function buildParamString(paramByKey) {
  return Object.entries(paramByKey).filter(([key, _]) => key !== "year").map(([key, value]) => {
...
function makeNomenEndpoint(year, code2, population) {
  ...
  const extension = "xml";
  if (code2 === "M14") {
    if (!population || population >= 3500)
      suffix = `_COM_SUP3500`;
    else if (population < 3500)
      suffix = `_COM_500_3500`;
    else if (population < 500)
      suffix = `_COM_INF500`;
  }
  let fileName = `${code2}${suffix}.${extension}`;
...
function getRecords(params) {
  ...
  return get(`${recordsUrl}/${endpoint}`).then(({ records }) => records.map((record) => record.fields));
}
var nomenUrl = "https://raw.githubusercontent.com/iOiurson/plans-de-compte/main";
...
var css$f = {
  code: ".svelte-jj00lc.svelte-jj00lc{color:white}.Search.svelte-jj00lc.svelte-jj00lc{border-radius:1rem;overflow:hidden;margin:2rem auto;max-width:75%;width:50rem;height:-webkit-fit-content;height:-moz-fit-content;height:fit-content}.searchbar.svelte-jj00lc.svelte-jj00lc{display:flex;background:#444;color:white;align-items:center;border-color:white}.searchbar.svelte-jj00lc.svelte-jj00lc:focus-within{background:#333}.searchbar.svelte-jj00lc .svelte-jj00lc{display:flex;justify-content:center;align-items:center}.searchbar.svelte-jj00lc .icon.svelte-jj00lc{margin:0 1rem;font-size:1.3rem}.searchbar.svelte-jj00lc span.icon.svelte-jj00lc{cursor:pointer}input.svelte-jj00lc.svelte-jj00lc{flex:1 0;padding:1rem;padding-left:0;outline:none;font-size:2rem;background:transparent;border:none;border-bottom:1px solid transparent}input.svelte-jj00lc.svelte-jj00lc::-moz-placeholder{color:#777}input.svelte-jj00lc.svelte-jj00lc:-ms-input-placeholder{color:#777}input.svelte-jj00lc.svelte-jj00lc::placeholder{color:#777}@media(max-width: 480px){input.svelte-jj00lc.svelte-jj00lc{font-size:1rem}}",
  map: `{"version":3,"file":"Search.svelte","sources":["Search.svelte"],"sourcesContent":["<script lang=\\"ts\\">var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {\\n    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }\\n    return new (P || (P = Promise))(function (resolve, reject) {\\n        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }\\n        function rejected(value) { try { step(generator[\\"throw\\"](value)); } catch (e) { reject(e); } }\\n        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }\\n        step((generator = generator.apply(thisArg, _arguments || [])).next());\\n    });\\n};\\nimport { onMount } from 'svelte';\\nexport let search;\\nexport let clear;\\nexport let selected = null;\\nlet value = '';\\nlet focus = false;\\n$: if (selected) {\\n    value = selected.nom;\\n}\\nonMount(() => __awaiter(void 0, void 0, void 0, function* () {\\n    search(value);\\n}));\\nfunction reset() {\\n    value = '';\\n    clear();\\n}\\nfunction setFocus(v) {\\n    focus = v;\\n}\\nfunction handleInput(e) {\\n    const target = e.target;\\n    const text = target.value;\\n    search(text);\\n    value = text;\\n}\\n<\/script>\\n\\n<div class=\\"Search\\">\\n  <div class=\\"searchbar\\" class:focus>\\n    <i class=\\"fas fa-search icon\\" />\\n    <input\\n      {value}\\n      on:input={handleInput}\\n      on:focus={() => setFocus(true)}\\n      on:blur={() => setFocus(false)}\\n      placeholder=\\"Entrez le nom d'une commune\\"\\n    />\\n    {#if value}\\n      <span class=\\"icon\\" on:click={reset}>\\n        <i class=\\"fas fa-times\\" />\\n      </span>\\n    {/if}\\n  </div>\\n  <slot />\\n</div>\\n\\n<style lang=\\"scss\\">* {\\n  color: white;\\n}\\n\\n.Search {\\n  border-radius: 1rem;\\n  overflow: hidden;\\n  margin: 2rem auto;\\n  max-width: 75%;\\n  width: 50rem;\\n  height: -webkit-fit-content;\\n  height: -moz-fit-content;\\n  height: fit-content;\\n}\\n\\n.searchbar {\\n  display: flex;\\n  background: #444;\\n  color: white;\\n  align-items: center;\\n  border-color: white;\\n}\\n.searchbar:focus-within {\\n  background: #333;\\n}\\n.searchbar * {\\n  display: flex;\\n  justify-content: center;\\n  align-items: center;\\n}\\n.searchbar .icon {\\n  margin: 0 1rem;\\n  font-size: 1.3rem;\\n}\\n.searchbar span.icon {\\n  cursor: pointer;\\n}\\n\\ninput {\\n  flex: 1 0;\\n  padding: 1rem;\\n  padding-left: 0;\\n  outline: none;\\n  font-size: 2rem;\\n  background: transparent;\\n  border: none;\\n  border-bottom: 1px solid transparent;\\n}\\ninput::-moz-placeholder {\\n  color: #777;\\n}\\ninput:-ms-input-placeholder {\\n  color: #777;\\n}\\ninput::placeholder {\\n  color: #777;\\n}\\n\\n@media (max-width: 480px) {\\n  input {\\n    font-size: 1rem;\\n  }\\n}</style>\\n"],"names":[],"mappings":"AAuDmB,4BAAE,CAAC,AACpB,KAAK,CAAE,KAAK,AACd,CAAC,AAED,OAAO,4BAAC,CAAC,AACP,aAAa,CAAE,IAAI,CACnB,QAAQ,CAAE,MAAM,CAChB,MAAM,CAAE,IAAI,CAAC,IAAI,CACjB,SAAS,CAAE,GAAG,CACd,KAAK,CAAE,KAAK,CACZ,MAAM,CAAE,mBAAmB,CAC3B,MAAM,CAAE,gBAAgB,CACxB,MAAM,CAAE,WAAW,AACrB,CAAC,AAED,UAAU,4BAAC,CAAC,AACV,OAAO,CAAE,IAAI,CACb,UAAU,CAAE,IAAI,CAChB,KAAK,CAAE,KAAK,CACZ,WAAW,CAAE,MAAM,CACnB,YAAY,CAAE,KAAK,AACrB,CAAC,AACD,sCAAU,aAAa,AAAC,CAAC,AACvB,UAAU,CAAE,IAAI,AAClB,CAAC,AACD,wBAAU,CAAC,cAAE,CAAC,AACZ,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,MAAM,CACvB,WAAW,CAAE,MAAM,AACrB,CAAC,AACD,wBAAU,CAAC,KAAK,cAAC,CAAC,AAChB,MAAM,CAAE,CAAC,CAAC,IAAI,CACd,SAAS,CAAE,MAAM,AACnB,CAAC,AACD,wBAAU,CAAC,IAAI,KAAK,cAAC,CAAC,AACpB,MAAM,CAAE,OAAO,AACjB,CAAC,AAED,KAAK,4BAAC,CAAC,AACL,IAAI,CAAE,CAAC,CAAC,CAAC,CACT,OAAO,CAAE,IAAI,CACb,YAAY,CAAE,CAAC,CACf,OAAO,CAAE,IAAI,CACb,SAAS,CAAE,IAAI,CACf,UAAU,CAAE,WAAW,CACvB,MAAM,CAAE,IAAI,CACZ,aAAa,CAAE,GAAG,CAAC,KAAK,CAAC,WAAW,AACtC,CAAC,AACD,iCAAK,kBAAkB,AAAC,CAAC,AACvB,KAAK,CAAE,IAAI,AACb,CAAC,AACD,iCAAK,sBAAsB,AAAC,CAAC,AAC3B,KAAK,CAAE,IAAI,AACb,CAAC,AACD,iCAAK,aAAa,AAAC,CAAC,AAClB,KAAK,CAAE,IAAI,AACb,CAAC,AAED,MAAM,AAAC,YAAY,KAAK,CAAC,AAAC,CAAC,AACzB,KAAK,4BAAC,CAAC,AACL,SAAS,CAAE,IAAI,AACjB,CAAC,AACH,CAAC"}`
};
var Search = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"Search svelte-jj00lc"}"><div class="${["searchbar svelte-jj00lc", ""].join(" ").trim()}"><i class="${"fas fa-search icon svelte-jj00lc"}"></i>
    <input${add_attribute("value", value, 0)} placeholder="${"Entrez le nom d'une commune"}" class="${"svelte-jj00lc"}">
    ${value ? `<span class="${"icon svelte-jj00lc"}"><i class="${"fas fa-times svelte-jj00lc"}"></i></span>` : ``}</div>
  ${slots.default ? slots.default({}) : ``}
</div>`;
});
var css$e = {
...
var Circle = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"circle svelte-10roc01"}" style="${"--size: " + escape(size) + escape(unit) + "; --color: " + escape(color) + "; --duration: " + escape(duration)}"></div>`;
});
var css$d = {
  code: ".Spinner.svelte-1hpylq3{display:flex;justify-content:center;align-items:center;flex:1 0}",
  map: `{"version":3,"file":"Spinner.svelte","sources":["Spinner.svelte"],"sourcesContent":["<script lang=\\"ts\\">import { Circle } from 'svelte-loading-spinners';\\nexport let color = 'white';\\nexport let size = '1';\\nexport let unit = 'rem';\\n<\/script>\\n\\n<div class=\\"Spinner\\">\\n  <Circle {color} {size} {unit} />\\n</div>\\n\\n<style lang=\\"scss\\">.Spinner {\\n  display: flex;\\n  justify-content: center;\\n  align-items: center;\\n  flex: 1 0;\\n}</style>\\n"],"names":[],"mappings":"AAUmB,QAAQ,eAAC,CAAC,AAC3B,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,MAAM,CACvB,WAAW,CAAE,MAAM,CACnB,IAAI,CAAE,CAAC,CAAC,CAAC,AACX,CAAC"}`
};
var Spinner = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"Spinner svelte-1hpylq3"}">${validate_component(Circle, "Circle").$$render($$result, { color, size, unit }, {}, {})}
</div>`;
});
var css$c = {
  code: "ul.svelte-117shey{margin:0}a.svelte-117shey{background:#555;display:flex;padding:0.5rem 1rem}a.svelte-117shey:hover{background:coral;color:white;cursor:pointer}.infos.svelte-117shey{display:flex}.other.svelte-117shey{margin-left:0.5rem;font-style:italic;opacity:0.3}.Suggestion.svelte-117shey .Spinner{justify-content:flex-end}",
  map: '{"version":3,"file":"Suggestions.svelte","sources":["Suggestions.svelte"],"sourcesContent":["<script lang=\\"ts\\">import Spinner from \'../../_components/Spinner.svelte\';\\nexport let suggestions = [];\\nexport let select;\\nexport let city = null;\\n<\/script>\\n\\n<ul>\\n  {#each suggestions as suggestion}\\n    <li class=\\"Suggestion\\" on:click={() => select(suggestion)}>\\n      <a\\n        href={`/budgets?name=${suggestion.nom}&insee=${suggestion.code}`}\\n        rel=\\"prefetch\\"\\n      >\\n        <div class=\\"infos\\">\\n          <div class=\\"name\\">{suggestion.nom}</div>\\n          {#if suggestion.departement}\\n            <div class=\\"other\\">\\n              {`${suggestion.departement.code} - ${suggestion.departement.nom}`}\\n            </div>\\n          {/if}\\n        </div>\\n\\n        {#if city && city.code === suggestion.code}\\n          <Spinner />\\n        {/if}\\n      </a>\\n    </li>\\n  {/each}\\n</ul>\\n\\n<style lang=\\"scss\\">ul {\\n  margin: 0;\\n}\\n\\na {\\n  background: #555;\\n  display: flex;\\n  padding: 0.5rem 1rem;\\n}\\n\\na:hover {\\n  background: coral;\\n  color: white;\\n  cursor: pointer;\\n}\\n\\n.infos {\\n  display: flex;\\n}\\n\\n.other {\\n  margin-left: 0.5rem;\\n  font-style: italic;\\n  opacity: 0.3;\\n}\\n\\n.Suggestion :global(.Spinner) {\\n  justify-content: flex-end;\\n}</style>\\n"],"names":[],"mappings":"AA8BmB,EAAE,eAAC,CAAC,AACrB,MAAM,CAAE,CAAC,AACX,CAAC,AAED,CAAC,eAAC,CAAC,AACD,UAAU,CAAE,IAAI,CAChB,OAAO,CAAE,IAAI,CACb,OAAO,CAAE,MAAM,CAAC,IAAI,AACtB,CAAC,AAED,gBAAC,MAAM,AAAC,CAAC,AACP,UAAU,CAAE,KAAK,CACjB,KAAK,CAAE,KAAK,CACZ,MAAM,CAAE,OAAO,AACjB,CAAC,AAED,MAAM,eAAC,CAAC,AACN,OAAO,CAAE,IAAI,AACf,CAAC,AAED,MAAM,eAAC,CAAC,AACN,WAAW,CAAE,MAAM,CACnB,UAAU,CAAE,MAAM,CAClB,OAAO,CAAE,GAAG,AACd,CAAC,AAED,0BAAW,CAAC,AAAQ,QAAQ,AAAE,CAAC,AAC7B,eAAe,CAAE,QAAQ,AAC3B,CAAC"}'
};
var Suggestions = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { suggestions = [] } = $$props;
  let { select } = $$props;
  let { city: city2 = null } = $$props;
  ...
  if ($$props.city === void 0 && $$bindings.city && city2 !== void 0)
    $$bindings.city(city2);
  $$result.css.add(css$c);
  return `<ul class="${"svelte-117shey"}">${each(suggestions, (suggestion) => `<li class="${"Suggestion svelte-117shey"}"><a${add_attribute("href", `/budgets?name=${suggestion.nom}&insee=${suggestion.code}`, 0)} rel="${"prefetch"}" class="${"svelte-117shey"}"><div class="${"infos svelte-117shey"}"><div class="${"name"}">${escape(suggestion.nom)}</div>
          ${suggestion.departement ? `<div class="${"other svelte-117shey"}">${escape(`${suggestion.departement.code} - ${suggestion.departement.nom}`)}
            </div>` : ``}</div>

        ${city2 && city2.code === suggestion.code ? `${validate_component(Spinner, "Spinner").$$render($$result, {}, {}, {})}` : ``}</a>
    </li>`)}
</ul>`;
});
var css$b = {
  code: "main.svelte-14ev131{flex:1 0;display:flex;padding-top:10%}",
  map: `{"version":3,"file":"index.svelte","sources":["index.svelte"],"sourcesContent":["<script lang=\\"ts\\">var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {\\n    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }\\n    return new (P || (P = Promise))(function (resolve, reject) {\\n        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }\\n        function rejected(value) { try { step(generator[\\"throw\\"](value)); } catch (e) { reject(e); } }\\n        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }\\n        step((generator = generator.apply(thisArg, _arguments || [])).next());\\n    });\\n};\\nimport { onMount } from 'svelte';\\nimport city from '../../stores/city';\\nimport { getCities } from '../../api';\\nimport Search from './_components/Search.svelte';\\nimport Suggestions from './_components/Suggestions.svelte';\\nonMount(() => __awaiter(void 0, void 0, void 0, function* () {\\n    select(null);\\n}));\\nlet citiesP;\\nlet previousCities;\\nfunction search(text) {\\n    console.log('TEXT', text);\\n    citiesP = fetchCities(text);\\n}\\nfunction clear() {\\n    citiesP = undefined;\\n}\\nfunction select(c) {\\n    city.set(c);\\n}\\nfunction fetchCities(text) {\\n    return __awaiter(this, void 0, void 0, function* () {\\n        return yield getCities(text).then(cities => {\\n            console.log('Villes', cities);\\n            const currentCities = cities.slice(0, 5);\\n            previousCities = currentCities;\\n            return currentCities;\\n        });\\n    });\\n}\\n<\/script>\\n\\n<svelte:head>\\n  <title>Livres ouverts</title>\\n</svelte:head>\\n\\n<main>\\n  <Search {search} {clear} selected={$city}>\\n    {#if citiesP}\\n      {#await citiesP}\\n        <Suggestions suggestions={previousCities} {select} />\\n      {:then cities}\\n        <Suggestions suggestions={cities} {select} city={$city} />\\n      {/await}\\n    {/if}\\n  </Search>\\n</main>\\n\\n<style lang=\\"scss\\">main {\\n  flex: 1 0;\\n  display: flex;\\n  padding-top: 10%;\\n}</style>\\n"],"names":[],"mappings":"AAyDmB,IAAI,eAAC,CAAC,AACvB,IAAI,CAAE,CAAC,CAAC,CAAC,CACT,OAAO,CAAE,IAAI,CACb,WAAW,CAAE,GAAG,AAClB,CAAC"}`
};
var Home = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_city();
  return `${$$result.head += `${$$result.title = `<title>Livres ouverts</title>`, ""}`, ""}

<main class="${"svelte-14ev131"}">${validate_component(Search, "Search").$$render($$result, { search, clear, selected: $city }, {}, {
    default: () => `${citiesP ? `${function(__value) {
      if (is_promise(__value))
        return `
        ${validate_component(Suggestions, "Suggestions").$$render($$result, { suggestions: previousCities, select }, {}, {})}
      `;
      return function(cities) {
        return `
        ${validate_component(Suggestions, "Suggestions").$$render($$result, { suggestions: cities, select, city: $city }, {}, {})}
      `;
      }(__value);
    }(citiesP)}` : ``}`
  })}
...
var goto = guard("goto");
var css$a = {
  code: ".Labels.svelte-g9cmc2.svelte-g9cmc2{position:relative;display:flex;flex-flow:column;width:15rem;margin:0}li.siren.svelte-g9cmc2.svelte-g9cmc2{border-top:1px solid rgba(255, 255, 255, 0.1);margin-bottom:0.5rem}li.siren.svelte-g9cmc2.svelte-g9cmc2:first-of-type{border:none}.siret.svelte-g9cmc2.svelte-g9cmc2{text-transform:capitalize;font-size:1rem;text-align:right;padding:0.25rem 0}.siret.svelte-g9cmc2>div.svelte-g9cmc2{opacity:0.2;cursor:pointer}.siret.svelte-g9cmc2>div.svelte-g9cmc2:hover{opacity:0.7}.siret.svelte-g9cmc2>div:hover .info.svelte-g9cmc2{opacity:0.4}.siret.svelte-g9cmc2>div:hover .siren.svelte-g9cmc2{opacity:1}.siret.selected.svelte-g9cmc2>div.svelte-g9cmc2{opacity:1;color:cornflowerblue}.siret.selected.svelte-g9cmc2>div .info.svelte-g9cmc2{opacity:0.4}.siret.selected.svelte-g9cmc2>div .siren.svelte-g9cmc2{opacity:1}.siret.svelte-g9cmc2 .info.svelte-g9cmc2{opacity:0.6;font-size:0.7rem}.siret.svelte-g9cmc2:not(.main) .siren.svelte-g9cmc2{opacity:0}.loading.svelte-g9cmc2.svelte-g9cmc2{position:absolute;top:0;left:0}",
  map: `{"version":3,"file":"Labels.svelte","sources":["Labels.svelte"],"sourcesContent":["<script lang=\\"ts\\">import Spinner from '../../_components/Spinner.svelte';\\nexport let labels;\\nexport let select;\\nexport let selected;\\nexport let loadingP;\\nconst defaultLabel = 'commune';\\n$: sirens = [...new Set(labels.map(({ siren }) => siren))];\\n<\/script>\\n\\n<ul class=\\"Labels\\">\\n  {#await loadingP}\\n    <div class=\\"loading\\">\\n      <Spinner color={'#999'} />\\n    </div>\\n  {/await}\\n  {#each sirens as siren}\\n    <li class=\\"siren\\">\\n      <ul>\\n        {#each labels.filter(l => l.siren === siren) as { siret, siren, etabl, label }, i}\\n          <li\\n            class=\\"siret\\"\\n            class:selected={selected === siret}\\n            class:main={i === 0}\\n          >\\n            <div on:click={() => select(siret)}>\\n              <div class=\\"info\\">\\n                <span class=\\"siren\\">{siren}</span>\\n                <span class=\\"etabl\\">{etabl}</span>\\n              </div>\\n              <div class=\\"label\\">{label || defaultLabel}</div>\\n            </div>\\n          </li>\\n        {/each}\\n      </ul>\\n    </li>\\n  {/each}\\n</ul>\\n\\n<style lang=\\"scss\\">.Labels {\\n  position: relative;\\n  display: flex;\\n  flex-flow: column;\\n  width: 15rem;\\n  margin: 0;\\n}\\n\\nli.siren {\\n  border-top: 1px solid rgba(255, 255, 255, 0.1);\\n  margin-bottom: 0.5rem;\\n}\\nli.siren:first-of-type {\\n  border: none;\\n}\\n\\n.siret {\\n  text-transform: capitalize;\\n  font-size: 1rem;\\n  text-align: right;\\n  padding: 0.25rem 0;\\n}\\n.siret > div {\\n  opacity: 0.2;\\n  cursor: pointer;\\n}\\n.siret > div:hover {\\n  opacity: 0.7;\\n}\\n.siret > div:hover .info {\\n  opacity: 0.4;\\n}\\n.siret > div:hover .siren {\\n  opacity: 1;\\n}\\n.siret.selected > div {\\n  opacity: 1;\\n  color: cornflowerblue;\\n}\\n.siret.selected > div .info {\\n  opacity: 0.4;\\n}\\n.siret.selected > div .siren {\\n  opacity: 1;\\n}\\n.siret .info {\\n  opacity: 0.6;\\n  font-size: 0.7rem;\\n}\\n.siret:not(.main) .siren {\\n  opacity: 0;\\n}\\n\\n.loading {\\n  position: absolute;\\n  top: 0;\\n  left: 0;\\n}</style>\\n"],"names":[],"mappings":"AAsCmB,OAAO,4BAAC,CAAC,AAC1B,QAAQ,CAAE,QAAQ,CAClB,OAAO,CAAE,IAAI,CACb,SAAS,CAAE,MAAM,CACjB,KAAK,CAAE,KAAK,CACZ,MAAM,CAAE,CAAC,AACX,CAAC,AAED,EAAE,MAAM,4BAAC,CAAC,AACR,UAAU,CAAE,GAAG,CAAC,KAAK,CAAC,KAAK,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CAC9C,aAAa,CAAE,MAAM,AACvB,CAAC,AACD,EAAE,kCAAM,cAAc,AAAC,CAAC,AACtB,MAAM,CAAE,IAAI,AACd,CAAC,AAED,MAAM,4BAAC,CAAC,AACN,cAAc,CAAE,UAAU,CAC1B,SAAS,CAAE,IAAI,CACf,UAAU,CAAE,KAAK,CACjB,OAAO,CAAE,OAAO,CAAC,CAAC,AACpB,CAAC,AACD,oBAAM,CAAG,GAAG,cAAC,CAAC,AACZ,OAAO,CAAE,GAAG,CACZ,MAAM,CAAE,OAAO,AACjB,CAAC,AACD,oBAAM,CAAG,iBAAG,MAAM,AAAC,CAAC,AAClB,OAAO,CAAE,GAAG,AACd,CAAC,AACD,oBAAM,CAAG,GAAG,MAAM,CAAC,KAAK,cAAC,CAAC,AACxB,OAAO,CAAE,GAAG,AACd,CAAC,AACD,oBAAM,CAAG,GAAG,MAAM,CAAC,MAAM,cAAC,CAAC,AACzB,OAAO,CAAE,CAAC,AACZ,CAAC,AACD,MAAM,uBAAS,CAAG,GAAG,cAAC,CAAC,AACrB,OAAO,CAAE,CAAC,CACV,KAAK,CAAE,cAAc,AACvB,CAAC,AACD,MAAM,uBAAS,CAAG,GAAG,CAAC,KAAK,cAAC,CAAC,AAC3B,OAAO,CAAE,GAAG,AACd,CAAC,AACD,MAAM,uBAAS,CAAG,GAAG,CAAC,MAAM,cAAC,CAAC,AAC5B,OAAO,CAAE,CAAC,AACZ,CAAC,AACD,oBAAM,CAAC,KAAK,cAAC,CAAC,AACZ,OAAO,CAAE,GAAG,CACZ,SAAS,CAAE,MAAM,AACnB,CAAC,AACD,oBAAM,KAAK,KAAK,CAAC,CAAC,MAAM,cAAC,CAAC,AACxB,OAAO,CAAE,CAAC,AACZ,CAAC,AAED,QAAQ,4BAAC,CAAC,AACR,QAAQ,CAAE,QAAQ,CAClB,GAAG,CAAE,CAAC,CACN,IAAI,CAAE,CAAC,AACT,CAAC"}`
};
...
var Labels = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  sirens = [...new Set(labels.map(({ siren }) => siren))];
  return `<ul class="${"Labels svelte-g9cmc2"}">${function(__value) {
    if (is_promise(__value))
      return `
    <div class="${"loading svelte-g9cmc2"}">${validate_component(Spinner, "Spinner").$$render($$result, { color: "#999" }, {}, {})}</div>
  `;
    return function() {
      return ``;
    }();
  }(loadingP)}
  ${each(sirens, (siren) => `<li class="${"siren svelte-g9cmc2"}"><ul>${each(labels.filter((l) => l.siren === siren), ({ siret, siren: siren2, etabl, label }, i) => `<li class="${[
    "siret svelte-g9cmc2",
    (selected === siret ? "selected" : "") + " " + (i === 0 ? "main" : "")
  ].join(" ").trim()}"><div class="${"svelte-g9cmc2"}"><div class="${"info svelte-g9cmc2"}"><span class="${"siren svelte-g9cmc2"}">${escape(siren2)}</span>
                <span class="${"etabl"}">${escape(etabl)}</span></div>
              <div class="${"label"}">${escape(label || defaultLabel)}</div></div>
          </li>`)}</ul>
    </li>`)}
</ul>`;
});
var css$9 = {
  code: ".Year.svelte-16rgqp8.svelte-16rgqp8{flex:1 0;width:5rem;display:flex;flex-flow:column;justify-content:flex-end;align-items:stretch;margin:0 0.5rem;font-size:0.8rem;color:white}.value.svelte-16rgqp8.svelte-16rgqp8{display:flex;padding:3px;height:1.5rem;flex-flow:column;align-items:center;background:#666;border-radius:8px;transition:height 0.5s ease-in-out}.Year.ready.svelte-16rgqp8.svelte-16rgqp8{cursor:pointer}.Year.ready.svelte-16rgqp8:hover .value.svelte-16rgqp8{background:coral}.Year.selected.svelte-16rgqp8 .value.svelte-16rgqp8{background:cornflowerblue}.Year.svelte-16rgqp8.svelte-16rgqp8:first-child{margin-left:0}.Year.svelte-16rgqp8.svelte-16rgqp8:last-child{margin-right:0}h3.svelte-16rgqp8.svelte-16rgqp8{font-size:1rem;margin-top:1rem;text-align:center}.info.svelte-16rgqp8.svelte-16rgqp8{display:flex;flex-flow:column;align-items:stretch;flex:1 0;justify-content:flex-end}.unavailable.svelte-16rgqp8.svelte-16rgqp8{opacity:0.1}.pending.svelte-16rgqp8.svelte-16rgqp8{opacity:0.2}.pending.svelte-16rgqp8 div.svelte-16rgqp8{padding:0}.pending.svelte-16rgqp8 .info.svelte-16rgqp8,.unavailable.svelte-16rgqp8 .info.svelte-16rgqp8{align-items:center;justify-content:center}.icon.svelte-16rgqp8.svelte-16rgqp8{color:transparent}",
  map: `{"version":3,"file":"Year.svelte","sources":["Year.svelte"],"sourcesContent":["<script lang=\\"ts\\">import { formatValue } from '../../../utils';\\nimport Spinner from '../../_components/Spinner.svelte';\\nexport let year;\\nexport let pending = false;\\nexport let value = null;\\nexport let maxP;\\nexport let select = null;\\nexport let selected = false;\\nlet height;\\nconst unavailable = !pending && !value;\\nconst ready = !!value;\\nif (maxP)\\n    maxP.then(max => {\\n        if (!unavailable) {\\n            setTimeout(() => (height = (value / max) * 100 + '%'), 50);\\n        }\\n    });\\nfunction _select() {\\n    if (ready)\\n        select === null || select === void 0 ? void 0 : select();\\n}\\n<\/script>\\n\\n<li\\n  class=\\"Year\\"\\n  class:pending\\n  class:unavailable\\n  class:ready\\n  class:selected\\n  on:click={_select}\\n>\\n  <div class=\\"info\\">\\n    {#if pending}\\n      <Spinner size=\\"1.5\\" />\\n    {:else if unavailable}\\n      <i class=\\"fas fa-times icon\\" />\\n    {:else if value}\\n      <div class=\\"value\\" style={\`height: \${height};\`}>\\n        {formatValue(value, 3)}\\n      </div>\\n    {/if}\\n  </div>\\n  <h3>{year}</h3>\\n</li>\\n\\n<style lang=\\"scss\\">.Year {\\n  flex: 1 0;\\n  width: 5rem;\\n  display: flex;\\n  flex-flow: column;\\n  justify-content: flex-end;\\n  align-items: stretch;\\n  margin: 0 0.5rem;\\n  font-size: 0.8rem;\\n  color: white;\\n}\\n\\n.value {\\n  display: flex;\\n  padding: 3px;\\n  height: 1.5rem;\\n  flex-flow: column;\\n  align-items: center;\\n  background: #666;\\n  border-radius: 8px;\\n  transition: height 0.5s ease-in-out;\\n}\\n\\n.Year.ready {\\n  cursor: pointer;\\n}\\n\\n.Year.ready:hover .value {\\n  background: coral;\\n}\\n\\n.Year.selected .value {\\n  background: cornflowerblue;\\n}\\n\\n.Year:first-child {\\n  margin-left: 0;\\n}\\n\\n.Year:last-child {\\n  margin-right: 0;\\n}\\n\\nh3 {\\n  font-size: 1rem;\\n  margin-top: 1rem;\\n  text-align: center;\\n}\\n\\n.info {\\n  display: flex;\\n  flex-flow: column;\\n  align-items: stretch;\\n  flex: 1 0;\\n  justify-content: flex-end;\\n}\\n\\n.unavailable {\\n  opacity: 0.1;\\n}\\n\\n.pending {\\n  opacity: 0.2;\\n}\\n\\n.pending div {\\n  padding: 0;\\n}\\n\\n.pending .info,\\n.unavailable .info {\\n  align-items: center;\\n  justify-content: center;\\n}\\n\\n.icon {\\n  color: transparent;\\n}\\n\\n.spinner {\\n  display: flex;\\n  align-items: center;\\n  flex: 1 0;\\n}</style>\\n"],"names":[],"mappings":"AA6CmB,KAAK,8BAAC,CAAC,AACxB,IAAI,CAAE,CAAC,CAAC,CAAC,CACT,KAAK,CAAE,IAAI,CACX,OAAO,CAAE,IAAI,CACb,SAAS,CAAE,MAAM,CACjB,eAAe,CAAE,QAAQ,CACzB,WAAW,CAAE,OAAO,CACpB,MAAM,CAAE,CAAC,CAAC,MAAM,CAChB,SAAS,CAAE,MAAM,CACjB,KAAK,CAAE,KAAK,AACd,CAAC,AAED,MAAM,8BAAC,CAAC,AACN,OAAO,CAAE,IAAI,CACb,OAAO,CAAE,GAAG,CACZ,MAAM,CAAE,MAAM,CACd,SAAS,CAAE,MAAM,CACjB,WAAW,CAAE,MAAM,CACnB,UAAU,CAAE,IAAI,CAChB,aAAa,CAAE,GAAG,CAClB,UAAU,CAAE,MAAM,CAAC,IAAI,CAAC,WAAW,AACrC,CAAC,AAED,KAAK,MAAM,8BAAC,CAAC,AACX,MAAM,CAAE,OAAO,AACjB,CAAC,AAED,KAAK,qBAAM,MAAM,CAAC,MAAM,eAAC,CAAC,AACxB,UAAU,CAAE,KAAK,AACnB,CAAC,AAED,KAAK,wBAAS,CAAC,MAAM,eAAC,CAAC,AACrB,UAAU,CAAE,cAAc,AAC5B,CAAC,AAED,mCAAK,YAAY,AAAC,CAAC,AACjB,WAAW,CAAE,CAAC,AAChB,CAAC,AAED,mCAAK,WAAW,AAAC,CAAC,AAChB,YAAY,CAAE,CAAC,AACjB,CAAC,AAED,EAAE,8BAAC,CAAC,AACF,SAAS,CAAE,IAAI,CACf,UAAU,CAAE,IAAI,CAChB,UAAU,CAAE,MAAM,AACpB,CAAC,AAED,KAAK,8BAAC,CAAC,AACL,OAAO,CAAE,IAAI,CACb,SAAS,CAAE,MAAM,CACjB,WAAW,CAAE,OAAO,CACpB,IAAI,CAAE,CAAC,CAAC,CAAC,CACT,eAAe,CAAE,QAAQ,AAC3B,CAAC,AAED,YAAY,8BAAC,CAAC,AACZ,OAAO,CAAE,GAAG,AACd,CAAC,AAED,QAAQ,8BAAC,CAAC,AACR,OAAO,CAAE,GAAG,AACd,CAAC,AAED,uBAAQ,CAAC,GAAG,eAAC,CAAC,AACZ,OAAO,CAAE,CAAC,AACZ,CAAC,AAED,uBAAQ,CAAC,oBAAK,CACd,2BAAY,CAAC,KAAK,eAAC,CAAC,AAClB,WAAW,CAAE,MAAM,CACnB,eAAe,CAAE,MAAM,AACzB,CAAC,AAED,KAAK,8BAAC,CAAC,AACL,KAAK,CAAE,WAAW,AACpB,CAAC"}`
};
var Year = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<li class="${[
    "Year svelte-16rgqp8",
    (pending ? "pending" : "") + " " + (unavailable ? "unavailable" : "") + " " + (ready ? "ready" : "") + " " + (selected ? "selected" : "")
  ].join(" ").trim()}"><div class="${"info svelte-16rgqp8"}">${pending ? `${validate_component(Spinner, "Spinner").$$render($$result, { size: "1.5" }, {}, {})}` : `${unavailable ? `<i class="${"fas fa-times icon svelte-16rgqp8"}"></i>` : `${value ? `<div class="${"value svelte-16rgqp8"}"${add_attribute("style", `height: ${height};`, 0)}>${escape(formatValue(value))}</div>` : ``}`}`}</div>
  <h3 class="${"svelte-16rgqp8"}">${escape(year)}</h3>
</li>`;
});
var css$8 = {
  code: ".Years.svelte-xkyjww{display:flex;justify-content:space-between;align-items:stretch;padding:1rem 2rem;margin:0;background:#333;height:11rem;width:-webkit-fit-content;width:-moz-fit-content;width:fit-content}",
  map: `{"version":3,"file":"Years.svelte","sources":["Years.svelte"],"sourcesContent":["<script lang=\\"ts\\">import Year from './Year.svelte';\\nexport let valuePs;\\nexport let years;\\nexport let select;\\nexport let selected;\\n$: maxP = Promise.all(valuePs).then(values => {\\n    const v = values.filter(v => v);\\n    return Math.max(...v);\\n});\\n<\/script>\\n\\n<ul class=\\"Years\\">\\n  {#each years as year, i}\\n    {#await valuePs[i]}\\n      <Year {year} pending={true} {maxP} />\\n    {:then value}\\n      <Year\\n        {year}\\n        {value}\\n        {maxP}\\n        selected={year === selected}\\n        select={() => select(year)}\\n      />\\n    {/await}\\n  {/each}\\n</ul>\\n\\n<style lang=\\"scss\\">.Years {\\n  display: flex;\\n  justify-content: space-between;\\n  align-items: stretch;\\n  padding: 1rem 2rem;\\n  margin: 0;\\n  background: #333;\\n  height: 11rem;\\n  width: -webkit-fit-content;\\n  width: -moz-fit-content;\\n  width: fit-content;\\n}</style>\\n"],"names":[],"mappings":"AA2BmB,MAAM,cAAC,CAAC,AACzB,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,aAAa,CAC9B,WAAW,CAAE,OAAO,CACpB,OAAO,CAAE,IAAI,CAAC,IAAI,CAClB,MAAM,CAAE,CAAC,CACT,UAAU,CAAE,IAAI,CAChB,MAAM,CAAE,KAAK,CACb,KAAK,CAAE,mBAAmB,CAC1B,KAAK,CAAE,gBAAgB,CACvB,KAAK,CAAE,WAAW,AACpB,CAAC"}`
};
var Years = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<ul class="${"Years svelte-xkyjww"}">${each(years2, (year, i) => `${function(__value) {
    if (is_promise(__value))
      return `
      ${validate_component(Year, "Year").$$render($$result, { year, pending: true, maxP }, {}, {})}
    `;
    return function(value) {
      return `
      ${validate_component(Year, "Year").$$render($$result, {
        year,
        value,
        maxP,
        selected: year === selected,
...
var tree = writable(void 0);
var code = writable(void 0);
var budget = writable(void 0);
...
var css$7 = {
  code: ".Csv.svelte-3cyy5m.svelte-3cyy5m{position:absolute;right:0;font-size:1.3rem;top:0;display:flex;align-items:center}a.svelte-3cyy5m.svelte-3cyy5m{display:flex;justify-content:center;align-items:center;width:2.2rem;height:2.2rem;border-radius:50%;cursor:pointer}a.svelte-3cyy5m i.svelte-3cyy5m{color:#ddd}a.svelte-3cyy5m.svelte-3cyy5m:hover{background:coral}a.svelte-3cyy5m:hover i.svelte-3cyy5m{background:coral;color:white}",
  map: `{"version":3,"file":"Csv.svelte","sources":["Csv.svelte"],"sourcesContent":["<script lang=\\"ts\\">import { makeCSV } from '../../../utils';\\nexport let data;\\n$: csvP = makeCSV(data);\\n<\/script>\\n\\n<div class=\\"Csv\\">\\n  {#await csvP then csv}\\n    <a href={csv.url} download={csv.file}>\\n      <i class=\\"fas fa-file-download\\" />\\n    </a>\\n  {/await}\\n</div>\\n\\n<style lang=\\"scss\\">.Csv {\\n  position: absolute;\\n  right: 0;\\n  font-size: 1.3rem;\\n  top: 0;\\n  display: flex;\\n  align-items: center;\\n}\\n\\na {\\n  display: flex;\\n  justify-content: center;\\n  align-items: center;\\n  width: 2.2rem;\\n  height: 2.2rem;\\n  border-radius: 50%;\\n  cursor: pointer;\\n}\\na i {\\n  color: #ddd;\\n}\\na:hover {\\n  background: coral;\\n}\\na:hover i {\\n  background: coral;\\n  color: white;\\n}</style>\\n"],"names":[],"mappings":"AAamB,IAAI,4BAAC,CAAC,AACvB,QAAQ,CAAE,QAAQ,CAClB,KAAK,CAAE,CAAC,CACR,SAAS,CAAE,MAAM,CACjB,GAAG,CAAE,CAAC,CACN,OAAO,CAAE,IAAI,CACb,WAAW,CAAE,MAAM,AACrB,CAAC,AAED,CAAC,4BAAC,CAAC,AACD,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,MAAM,CACvB,WAAW,CAAE,MAAM,CACnB,KAAK,CAAE,MAAM,CACb,MAAM,CAAE,MAAM,CACd,aAAa,CAAE,GAAG,CAClB,MAAM,CAAE,OAAO,AACjB,CAAC,AACD,eAAC,CAAC,CAAC,cAAC,CAAC,AACH,KAAK,CAAE,IAAI,AACb,CAAC,AACD,6BAAC,MAAM,AAAC,CAAC,AACP,UAAU,CAAE,KAAK,AACnB,CAAC,AACD,eAAC,MAAM,CAAC,CAAC,cAAC,CAAC,AACT,UAAU,CAAE,KAAK,CACjB,KAAK,CAAE,KAAK,AACd,CAAC"}`
};
var Csv = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  csvP = makeCSV(data);
  return `<div class="${"Csv svelte-3cyy5m"}">${function(__value) {
    if (is_promise(__value))
      return ``;
    return function(csv) {
      return `
    <a${add_attribute("href", csv.url, 0)}${add_attribute("download", csv.file, 0)} class="${"svelte-3cyy5m"}"><i class="${"fas fa-file-download svelte-3cyy5m"}"></i></a>
  `;
    }(__value);
  }(csvP)}
</div>`;
...
var css$6 = {
  code: ".path.svelte-74381w{flex:1 0;display:flex;flex-flow:column-reverse}.steps.svelte-74381w{display:flex;font-size:0.9rem}.next.svelte-74381w{margin:0 0.5rem}.step.svelte-74381w:hover{color:coral;cursor:pointer}.current.svelte-74381w{font-size:2rem;line-height:1.8rem}",
  map: '{"version":3,"file":"Path.svelte","sources":["Path.svelte"],"sourcesContent":["<script lang=\\"ts\\">var _a;\\nexport let steps;\\n$: current = (_a = steps.pop()) === null || _a === void 0 ? void 0 : _a.label;\\n<\/script>\\n\\n<div class=\\"path\\">\\n  {#if current}\\n    <div class=\\"current\\">{current}</div>\\n  {/if}\\n  <div class=\\"steps\\">\\n    {#each steps as { label, select }}\\n      <div class=\\"step\\" on:click={select}>\\n        {label}\\n        <i class=\\"fas fa-caret-right next\\" />\\n      </div>\\n    {/each}\\n  </div>\\n</div>\\n\\n<style lang=\\"scss\\">.path {\\n  flex: 1 0;\\n  display: flex;\\n  flex-flow: column-reverse;\\n}\\n\\n.steps {\\n  display: flex;\\n  font-size: 0.9rem;\\n}\\n\\n.next {\\n  margin: 0 0.5rem;\\n}\\n\\n.step:hover {\\n  color: coral;\\n  cursor: pointer;\\n}\\n\\n.current {\\n  font-size: 2rem;\\n  line-height: 1.8rem;\\n}</style>\\n"],"names":[],"mappings":"AAmBmB,KAAK,cAAC,CAAC,AACxB,IAAI,CAAE,CAAC,CAAC,CAAC,CACT,OAAO,CAAE,IAAI,CACb,SAAS,CAAE,cAAc,AAC3B,CAAC,AAED,MAAM,cAAC,CAAC,AACN,OAAO,CAAE,IAAI,CACb,SAAS,CAAE,MAAM,AACnB,CAAC,AAED,KAAK,cAAC,CAAC,AACL,MAAM,CAAE,CAAC,CAAC,MAAM,AAClB,CAAC,AAED,mBAAK,MAAM,AAAC,CAAC,AACX,KAAK,CAAE,KAAK,CACZ,MAAM,CAAE,OAAO,AACjB,CAAC,AAED,QAAQ,cAAC,CAAC,AACR,SAAS,CAAE,IAAI,CACf,WAAW,CAAE,MAAM,AACrB,CAAC"}'
};
var Path = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  current = (_a = steps.pop()) === null || _a === void 0 ? void 0 : _a.label;
  return `<div class="${"path svelte-74381w"}">${current ? `<div class="${"current svelte-74381w"}">${escape(current)}</div>` : ``}
  <div class="${"steps svelte-74381w"}">${each(steps, ({ label, select }) => `<div class="${"step svelte-74381w"}">${escape(label)}
        <i class="${"fas fa-caret-right next svelte-74381w"}"></i>
      </div>`)}</div>
</div>`;
});
var css$5 = {
...
var DebitOrCredit = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let max;
  let makeCustomStyle;
  let { credit } = $$props;
  let { debit } = $$props;
  ...
  return `<div class="${"debit-credit svelte-1505gru"}"><div class="${"credit svelte-1505gru"}"><div class="${"value-container svelte-1505gru"}"><div class="${[
    ...
    ""
  ].join(" ").trim()}"${add_attribute("style", makeCustomStyle(credit), 0)}>${escape(formatValue(credit))}</div></div>
    <h4 class="${"svelte-1505gru"}">Recettes</h4></div>
  <div class="${"debit svelte-1505gru"}"><div class="${"value-container svelte-1505gru"}"><div class="${["value svelte-1505gru", ""].join(" ").trim()}"${add_attribute("style", makeCustomStyle(debit), 0)}>${escape(formatValue(debit))}</div></div>
...
var css$4 = {
  code: ".bar.svelte-8ob999.svelte-8ob999{position:relative;display:flex;align-items:stretch;padding:0.5rem 1rem;margin:0.5rem;height:5rem}.clickable.svelte-8ob999.svelte-8ob999:hover{cursor:pointer}.clickable.svelte-8ob999:hover .background.svelte-8ob999{background:coral}.labels.svelte-8ob999.svelte-8ob999{display:flex;flex-flow:column;justify-content:space-between;width:100%;z-index:1}h3.svelte-8ob999.svelte-8ob999{font-size:1.1rem}.values.svelte-8ob999.svelte-8ob999{display:flex;justify-content:space-between;align-items:baseline}.value.svelte-8ob999.svelte-8ob999{font-size:2rem;line-height:1.8rem}.background.svelte-8ob999.svelte-8ob999{position:absolute;top:0;left:0;height:100%;background:#ed64a3;border-radius:8px}",
  map: '{"version":3,"file":"Bar.svelte","sources":["Bar.svelte"],"sourcesContent":["<script lang=\\"ts\\">import { format } from \'d3-format\';\\nimport { formatValue } from \'../../../utils\';\\nexport let value;\\nexport let percentage;\\nexport let width;\\nexport let label;\\nexport let handleClick = undefined;\\n$: clickable = !!handleClick;\\n<\/script>\\n\\n<div class=\\"bar\\" class:clickable on:click={() => handleClick?.()}>\\n  <div class=\\"labels\\">\\n    <h3>\\n      {label}\\n      {#if clickable}\\n        <i class=\\"fas fa-ellipsis-h\\" />\\n      {/if}\\n    </h3>\\n    <div class=\\"values\\">\\n      <span class=\\"value\\">{`${formatValue(value)}`}</span>\\n      <span class=\\"percentage\\">{`${format(\'.0%\')(percentage)}`}</span>\\n    </div>\\n\\n  </div>\\n  <div class=\\"background\\" style={`width: ${width * 100}%`} />\\n</div>\\n\\n<style lang=\\"scss\\">.bar {\\n  position: relative;\\n  display: flex;\\n  align-items: stretch;\\n  padding: 0.5rem 1rem;\\n  margin: 0.5rem;\\n  height: 5rem;\\n}\\n\\n.clickable:hover {\\n  cursor: pointer;\\n}\\n.clickable:hover .background {\\n  background: coral;\\n}\\n\\n.labels {\\n  display: flex;\\n  flex-flow: column;\\n  justify-content: space-between;\\n  width: 100%;\\n  z-index: 1;\\n}\\n\\nh3 {\\n  font-size: 1.1rem;\\n}\\n\\n.values {\\n  display: flex;\\n  justify-content: space-between;\\n  align-items: baseline;\\n}\\n\\n.value {\\n  font-size: 2rem;\\n  line-height: 1.8rem;\\n}\\n\\n/* .percentage {\\n  font-size: 1.5rem;\\n} */\\n.background {\\n  position: absolute;\\n  top: 0;\\n  left: 0;\\n  height: 100%;\\n  background: #ed64a3;\\n  border-radius: 8px;\\n}</style>\\n"],"names":[],"mappings":"AA2BmB,IAAI,4BAAC,CAAC,AACvB,QAAQ,CAAE,QAAQ,CAClB,OAAO,CAAE,IAAI,CACb,WAAW,CAAE,OAAO,CACpB,OAAO,CAAE,MAAM,CAAC,IAAI,CACpB,MAAM,CAAE,MAAM,CACd,MAAM,CAAE,IAAI,AACd,CAAC,AAED,sCAAU,MAAM,AAAC,CAAC,AAChB,MAAM,CAAE,OAAO,AACjB,CAAC,AACD,wBAAU,MAAM,CAAC,WAAW,cAAC,CAAC,AAC5B,UAAU,CAAE,KAAK,AACnB,CAAC,AAED,OAAO,4BAAC,CAAC,AACP,OAAO,CAAE,IAAI,CACb,SAAS,CAAE,MAAM,CACjB,eAAe,CAAE,aAAa,CAC9B,KAAK,CAAE,IAAI,CACX,OAAO,CAAE,CAAC,AACZ,CAAC,AAED,EAAE,4BAAC,CAAC,AACF,SAAS,CAAE,MAAM,AACnB,CAAC,AAED,OAAO,4BAAC,CAAC,AACP,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,aAAa,CAC9B,WAAW,CAAE,QAAQ,AACvB,CAAC,AAED,MAAM,4BAAC,CAAC,AACN,SAAS,CAAE,IAAI,CACf,WAAW,CAAE,MAAM,AACrB,CAAC,AAKD,WAAW,4BAAC,CAAC,AACX,QAAQ,CAAE,QAAQ,CAClB,GAAG,CAAE,CAAC,CACN,IAAI,CAAE,CAAC,CACP,MAAM,CAAE,IAAI,CACZ,UAAU,CAAE,OAAO,CACnB,aAAa,CAAE,GAAG,AACpB,CAAC"}'
};
var Bar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  clickable = !!handleClick;
  return `<div class="${["bar svelte-8ob999", clickable ? "clickable" : ""].join(" ").trim()}"><div class="${"labels svelte-8ob999"}"><h3 class="${"svelte-8ob999"}">${escape(label)}
      ${clickable ? `<i class="${"fas fa-ellipsis-h"}"></i>` : ``}</h3>
    <div class="${"values svelte-8ob999"}"><span class="${"value svelte-8ob999"}">${escape(`${formatValue(value)}`)}</span>
      <span class="${"percentage"}">${escape(`${format2(".0%")(percentage)}`)}</span></div></div>
  <div class="${"background svelte-8ob999"}"${add_attribute("style", `width: ${width * 100}%`, 0)}></div>
</div>`;
});
var css$3 = {
...
var Chart = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  total = sumBy(values, "value");
  max = Math.max(...values.map(({ value }) => value));
  return `<div class="${"chart svelte-1tuxdri"}">${each(values, ({ label, value, handleClick }) => `${validate_component(Bar, "Bar").$$render($$result, {
    label,
    value,
    percentage: value / total,
    width: value / max,
...
var Summary = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  {
    budgetP.then((b) => __awaiter2(void 0, void 0, void 0, function* () {
      if (b) {
        const { year: year2, nomen: code2 } = b;
        budget.set(b);
        const _tree = yield getFonctionTree(year2, code2, $city === null || $city === void 0 ? void 0 : $city.population);
        const aggTree = _tree && aggregateData(b.records, _tree);
        tree.set(aggTree);
      }
    }));
  }
  steps = $code && $tree ? stepsFromString($code).map((code2) => {
  ...
  infosP = budgetP.then((budget2) => {
    if (budget2) {
      const main = type && ($fonction ? $fonction[type] : budget2[type]);
      return {
  ...
  $$unsubscribe_code();
  $$unsubscribe_city();
  return `<div class="${"summary svelte-1e52ag5"}"><header class="${"svelte-1e52ag5"}"><h3 class="${["svelte-1e52ag5", steps.length > 0 ? "clickable" : ""].join(" ").trim()}">${escape(year)}</h3>
    ${$tree ? `${validate_component(Path, "Path").$$render($$result, { steps }, {}, {})}` : ``}
    ${function(__value) {
    if (is_promise(__value))
      return ``;
    return function(budget2) {
      return `
      ${budget2 ? `${validate_component(Csv, "Csv").$$render($$result, { data: budget2 }, {}, {})}` : ``}
    `;
    }(__value);
  }(budgetP)}</header>
  <div class="${"values svelte-1e52ag5"}">${function(__value) {
    if (is_promise(__value))
      return `
      ${validate_component(Spinner, "Spinner").$$render($$result, { color: "#333", size: "3" }, {}, {})}
    `;
    return function(infos) {
      return `
      ${!infos ? `<div class="${"none svelte-1e52ag5"}">Aucun budget</div>` : `${!type ? `${validate_component(DebitOrCredit, "DebitOrCredit").$$render($$result, {
        credit: infos.credit,
        debit: infos.debit,
        select: selectType
      }, {}, {})}` : `${values ? `<div class="${"main svelte-1e52ag5"}">${escape(formatValue(infos.main))}</div>
        ${validate_component(Chart, "Chart").$$render($$result, { values }, {}, {})}` : ``}`}`}
      ${infos ? `<div class="${"nomen svelte-1e52ag5"}">${escape(infos.nomen)}</div>` : ``}
    `;
    }(__value);
  }(infosP)}</div>
</div>`;
...
function load({ page: { query } }) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (!siret || !sirens) {
      const siretsFromInsee = yield getSiretsFromInsee(name, insee);
      sirens = extractSirens(siretsFromInsee);
      const sirets = siretsFromInsee.filter((e) => e.etablissementSiege).map((e) => e.siret).sort();
      siret = sirets[0];
    }
    return {
      props: {
...
var Budgets = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const cityP = $city ? Promise.resolve($city) : getCity(insee).then((result) => {
    city.set(result);
    return result;
  });
  let budgetPs = years.map((year) => getRecords({ ident: [currentSiret], year }).catch(() => []).then((records) => {
    const b = makeBudget({
      city: name,
      siret: currentSiret,
      year,
  ...
  const otherBudgetPs = [...years].reverse().map((year) => getRecords({ siren: sirens, year }).catch(() => []).then((records) => orderRecordsBySiret(records).map(({ siret, records: records2 }) => {
    const b = makeBudget({ city: name, siret, year, records: records2 });
    const id = makeId(siret, year);
    if (siret !== currentSiret)
      budgetById[id] = b;
    return b;
  ...
  if ($$props.currentSiret === void 0 && $$bindings.currentSiret && currentSiret !== void 0)
  ...
  findSimilarBudget = function(siret) {
    return Object.values(budgetById).find((budget2) => budget2 && budget2.siret === siret);
  };
  findSimilarLabel = function() {
    const id = makeId(currentSiret, currentYear);
  ...
  sirets = [...new Set(Object.values(budgetById).filter((b) => b).map((b) => b.siret))].sort();
  labels = sirets.map((s2) => {
    const id = makeId(s2, currentYear);
    const budget2 = budgetById[id];
    return budget2 || findSimilarBudget(s2);
  }).filter((l) => l);
  valuePs = budgetPs.map((budgetP2) => budgetP2.then((budget2) => budget2 && budget2.obnetcre));
  yearIndex = years.findIndex((y) => y === currentYear);
  budgetP = budgetPs[yearIndex];
  label = findSimilarLabel();
  $$unsubscribe_city();
  ...
  `;
    return function(city2) {
      return `
    ${city2 ? `${$$result.title = `<title>${escape(`Budgets pour ${name} (${city2.departement.code})`)}</title>`, ""}` : ``}
  `;
    }(__value);
  }(cityP)}`, ""}

<header class="${"svelte-1u2u6o1"}"><a class="${"home svelte-1u2u6o1"}" href="${"/"}"><i class="${"fas fa-book-open"}"></i></a>
  <div class="${"info svelte-1u2u6o1"}"><div class="${"labels svelte-1u2u6o1"}"><h1 class="${"svelte-1u2u6o1"}">${escape(name)}</h1>

      ${label ? `<h2 class="${"svelte-1u2u6o1"}">${escape(label)}</h2>` : ``}</div>

    <div class="${"departement svelte-1u2u6o1"}">${function(__value) {
    if (is_promise(__value))
      return `
    ...
    return function(city2) {
      return `
        ${city2 ? `<div>${escape(`${city2.departement.code} - ${city2.departement.nom}`)}</div>` : ``}
      `;
    }(__value);
  }(cityP)}</div></div></header>

<div class="${"content svelte-1u2u6o1"}"><menu class="${"svelte-1u2u6o1"}">${validate_component(Labels, "Labels").$$render($$result, {
    labels,
    loadingP,
    selected: currentSiret,
    select: selectSiret
  }, {}, {})}</menu>
  <div class="${"dataviz svelte-1u2u6o1"}">${validate_component(Years, "Years").$$render($$result, {
    years,
    valuePs,
    selected: currentYear,
    select: selectYear
  }, {}, {})}
    ${validate_component(Summary, "Summary").$$render($$result, { year: currentYear, budgetP }, {}, {})}</div>
</div>`;
});
...
var About = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `${$$result.head += `${$$result.title = `<title>\xC0 propos de Livres ouverts</title>`, ""}`, ""}
